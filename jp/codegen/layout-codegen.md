...
### 機能コンポーネント
場合によっては、関連するコンポーネントが現在のレイヤーの外側にあり (必ずしも兄弟であるとは限らない)、現在のレイヤーと交差する必要があります。
たとえば、以下に示すように、`Grid Features/Column Moving` をグリッドの上に配置して、そのグリッドの列を移動できます。

<img class="responsive-img" src="../images/layout_codegen_ext_comps.png" />

これらのグリッド機能はすべて個別に使用することはできず、グリッドを有効にする必要があることに注意してください。同じことが TimePicker/DatePicker にも当てはまります。関連するコンポーネントに機能を追加するには、TimePicker/DatePicker 入力が必要です。(DatePicker または TimePicker のみを別のコンポーネントとして使用する場合は、Calendar を使用することをお勧めします。)
