---
title: レイアウトの生成 
_description: Sketch レイアウトおよびサイズ変更が生成されたコードおよび動作になる説明。
...
デザイナーが web 開発者が使用されているデフォルト色パレットと一致する場合、Ignite UI コンポーネントが色をデフォルト パレットから取得するため、HTML でテーマ色の CSS および追加の div 要素である必要はありません。
...
コード生成が CSS クラス名を作成している場合、CSS に適用する Sketch 要素の名前を CSS 名として使用します。_Sketch 描画で要素に意味のある名前を設定すると、よりわかりやすくなり、生成されている HTML および CSS の編集が可能になります。_名前が Sketch のデフォルト名の場合、CSS が分かりにくくなります。
...
コード生成は、テーマ色またはフォントが指定される場合に Ignite UI コンポーネントを div にラップします。これでコード生成がプロジェクトのデフォルト パレットを使用できます。[Ignite UI Theming](https://jp.infragistics.com/products/ignite-ui-angular/angular/components/themes/index.html) で色パレットを変更すると、ブランディングを簡単に変更できます。
...
コード生成から_一貫性のあるレイアウト_を取得するために[レイアウトのベスト プラクティス](best-layout-practices.md)を参照してください。このトピックは Sketch デザインの全般的なレイアウト シナリオを説明します。Sketch のレイアウト要件を HTML と CSS レイアウトに移動するガイダンスを提供します。このトピックはコード生成が Sketch ファイルのレイアウトを処理する方法を説明します。
...
以下の画像で 2 つのグループ (div) が動的に生成されます。1 つは州、市、郵便番号を含み、1 つは 2 つのボタンを含みます。名前が行の単一の子であるため、div は作成されません。
...
レイアウト グループがグループに含まれない要素のために作成される場合、動的に生成された div (行) に追加される Group がある場合、生成されたレイアウトは無効です。以下の画像で、上の Artboard は一貫性のあるレイアウトを生成するために正しいグループ化を紹介します。Battleship および Actions は ImageButtons グループにグループ化されます。
...
Sketch デザイナーはグループまたはアートボードの背景として塗りつぶされた長方形を使用します。このベータ版で Indigo-Styling Color 要素を以下のように使用できます。このように使用される色の高さおよび幅を親に相対して +- 3 に設定する必要があります。以下の Artboard および Group の色要素の使用に注意してください。
...
Sketch 描画のシナリオでレイアウトの CSS の生成のときに Sketch および HTML/CSS の間に 1:1 マッピングがない場合があります。親コンテナーがサイズ変更するときに Sketch で要素をサイズ変更しないために要素の高さを固定する場合に問題が可能です。要素の高さが固定されますが、Web 開発者が CSS を作成する方法と一致しない場合があります。高さを設定する代わりに、複数の HTML 要素のデフォルト高さが使用されます。
コード生成で、以下のコントロールはコントロールの HTML によって提供されるデフォルトの高さを使用して描画します。つまり、コード生成で固定高さが無視され、この要素のために高さ CSS が生成されません。
...
要素の幅の計算で以下の Sketch 設定が影響します。また、設定しない場合、別の要素は要素のタイプ、行で単一の子または行で子のグループにあるかどうかに基づいて別のルールに適用されます。
...
| Sketch のレイアウト設定|幅                                                                                                                                                                                                         |
| ------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| <img src="../images/layout_codegen_table1.png" /> |デフォルトの状態です。要素が行で単一の子の場合、幅は 100% です。要素が複数の要素がある行にある場合、以下の比較的な幅サイズ変更ルールが適用されます。 |
| <img src="../images/layout_codegen_table2.png" /> |幅は固定されます。                                                                                                                                                                                          |
| <img src="../images/layout_codegen_table3.png" /> |要素が行で単一の子である場合、その幅は 100% です。要素が複数の要素がある行にある場合、以下の比較的な幅サイズ変更ルールが適用されます。                           |
...
上記の例外以外の要素で、幅が Sketch で固定する場合、幅は生成されたコードで固定されます。
...
上記の例外以外の要素で、幅が固定されない場合、要素は比較的な幅サイズを使用します。
...
要素の高さが固定され、上下のマージンが等しく (+-3px) 要素は上下にピン固定される場合、要素が親に垂直に中央揃えになります。

<img src="../images/layout_codegen12.png" />

### 水平方向

要素の幅が固定され、左右のマージンが等しく (+-3px) 要素は左右にピン固定される場合、要素が親に水平に中央揃えになります。

<img src="../images/layout_codegen13.png" />

### 両方

要素の高さと幅が固定され、左右と上下の距離が等しく (+-3px) 左右にピン固定されまる要素がない場合、オブジェクトを親で中央揃えになります。
...
2 つ目の例では、3 つ目の要素 (ボタン) がグループに追加されています。ボタンはその他の配置ルールを使用して配置され、バッジの絶対位置の一部ではありません。2 番目の場合はサポートされますが、絶対位置の重複を要求する要素はグループ内で 1 つのみにしてください。
...
要素が右下のみにピン固定されている場合、右配置および下配置は CSS に追加されます。
...
Sketch 画像はサポートされます。画像は、Sketch で固定した高さと幅で描画されます。画像の幅がアートボードの表示範囲を超える場合、コード生成で画像をクリップしないことに注意してください。
