...
提供された場合、Headers は描画する列数を決定します。Header を下にある Body (Cell) と一致させるために列を作成します。一致する Body が見つからない場合、列は Header の情報に基づいて作成されます。一致が見つかった場合、情報が Header から取得され、次に Body/Cell から取得されます。
...
提供された場合、Bodies (セル) は描画する列の数を決定します。Header を Body (Cell) と一致しようとします。一致する Header が見つからない場合、列は Body/Cell の情報に基づいて作成されます。一致が見つかった場合、情報が Header から取得され、次に Body/Cell から取得されます。
