...
The Bullet Graph comes with three preset range configurations: two, three, and four. In Figma you can change the Series amount using the `Series Amount` property in the properties panel on the right. In Sketch, this is achieved with `Symbol Overrides`, while in Adobe XD, we use the `Component States` paradigm to let you easily switch between them.
