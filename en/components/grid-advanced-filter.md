...
The Grid has three `Advanced Filtering` symbols under `Features`, one for each Display Density, that represent the overlay shown over the Grid. In Figma you can switch between the different display densities via the dedicated property in the property panel. In Adobe XD you can switch between display densities through the `Component States`.
