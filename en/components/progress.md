...
When using a Determinate type of Progress, you may choose the Value between six percentage variants. In Figma you can select between the variants from `Value` property in the right sidebar. In Sketch, this is achieved with `Symbol Overrides` to let you easily switch between them, while in Adobe XD you have to drag the desired amount from the `Libraries` panel and drop it on top of the `Progress Amount` layer to change the default value.
